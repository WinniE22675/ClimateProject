...
# Tune these based on your data resolution and available RAM.
# Rule of thumb: each chunk should be ~100–500MB in memory.
...
def core_process_file(raw_path, save_path, scope):
    try:
        ...
        with xr.open_dataset(raw_path, chunks=CHUNK_CONFIG) as ds:
            ...
            # Clip spatially and temporally (still lazy — no data loaded yet)
            ...
            # REMOVED ds_subset.load() — never materialize the full array.
            # Unit conversions below work lazily on Dask arrays.
            # ensure_pr_unit / ensure_temperature_unit must return xr.DataArray
            # (arithmetic ops like * or + are lazy on Dask — no change needed there).
        
            for var in list(ds_subset.data_vars):
                if var == "pr":
            ...
            # ds_subset.encoding = {} 
            # Clear encoding so xarray doesn't reuse source file's compression settings
            ds_subset = ds_subset.copy()  # shallow copy to safely mutate encoding
            for var in ds_subset.variables:
                ds_subset[var].encoding = {}
                ds_subset[var].attrs.pop("_FillValue", None)
                # if "_FillValue" in ds_subset[var].attrs:
                #     del ds_subset[var].attrs["_FillValue"]

            # ds_subset.to_netcdf(save_path, format='NETCDF4')
            # Peak RAM = 1 chunk at a time, NOT the whole dataset.
            ds_subset.to_netcdf(save_path, format="NETCDF4", compute=True)
        ...
        # Clean up partial output file if write failed
...
def process_and_clip(user_id: str, slot_id: int, dataset_name: str, scope):
    ...
    for filename in files:
        ...
        # Force GC between files so Dask worker memory is released
        gc.collect()
